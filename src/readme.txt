To set material: 
	1. EELS VarName = Material('MaterialName',InitialEnergy);
		Mat = Material('Au',2000);
	2. XPS VarName = XPSMaterial('MaterialName',ShellNumber,'AnodeName');
	   ...
	   In the case of Au: 1 shell is 4s1/2, 2 - 4p1/2, 3 - 4p3/2 etc.
		...
		Mat = XPSMaterial('Au',1);
           It is also possible to write this: 
	        VarName = XPSMaterial('MaterialName',ShellNumber,AnodeEnergy);
		Mat = XPSMaterial('Au',1,1486.6);
To calculate:

	We have three possible methods to calculate energy spectra:
	...
	2. Transmission (TEELS) calculation
...
To calculate a multilayer system (for example surface and bulk):
	1. EELS
		...
		Methods = {'SLA','NS'};
                Methods = {'NS','NS'};
		NS method is the default one.
		The number of pointed methods should coincide with the number of layers.
